...
class DraftApp(Frame):
    ...
    def __init__(self, df=None, parent=None):
        ...
        for i in range(num_rounds):
            turns += (
                reversed(range(num_competitors)) if i % 2 else range(num_competitors)
            )
        ...
        while state.GetMoves() != [] and draft_type != "sim":
            if turns[0] == draft_position:
                ...
                while illegal == 1:
                    ...
                    if (
                        manualMove in vis_df["name"].values
                    ):  # vis_df['name'].str.contains(manualMove).any()
                        illegal = 0
                    else:
        ...
        pd.DataFrame(
            {"Team " + str(i + 1): r for i, r in enumerate(state.rosters)}
        ).to_csv("Projections\\sim_results.csv")
...
class DraftState:
    ...
    def GetResult(self, playerjm):
        ...
        pos_wgts = {
            ("QB"): [0.6, 0.4],  # .6
            ("WR"): [0.7, 0.7, 0.4, 0.2],  # .7
            ("RB"): [0.7, 0.7, 0.4, 0.2],  # .7
            ("TE"): [0.6, 0.4],  # .6
            ("RB", "WR", "TE"): [0.6, 0.4],  # .6
            ("D/ST"): [0.5, 0.3, 0.1],  # .5
            ("K"): [0.5, 0.2, 0.2, 0.1],  # .4
        ...
        for pos, wgts in pos_wgts.items():
            result += np.mean(
                [p.points for p in self.freeagents if p.position in pos][:3]
            ) * sum(wgts)
        return result
    ...
    def Clone(self):
        ...
        st = DraftState(
            rosters, self.turns[:], self.freeagents[:], self.playerJustMoved
        )
        return st
